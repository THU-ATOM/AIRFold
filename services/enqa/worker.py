...
from lib.tool.enqa import EnQA
...
celery.conf.task_routes = {
    "worker.*": {"queue": "queue_enqa"},
}


@celery.task(name="enqa")
def enqaTask(requests: List[Dict[str, Any]]):
...
class BlastRunner(BaseCommandRunner):
    ...
    def build_command(self, request: Dict[str, Any]) -> str:
        ...
        command = "".join(
            [
                f"python {pathtool.get_module_path(EnQA)} ",
                f"-i {ptree.seq.fasta} ",
