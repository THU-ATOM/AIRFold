...
class ProteinBertModel(nn.Module):
    ...
    def __init__(self, args: Namespace, alphabet):
        super().__init__()
        self.args = vars(args)
        self.alphabet_size = len(alphabet)
        self.padding_idx: Optional[Tensor] = alphabet.padding_idx
    ...
    def forward(self, tokens, need_head_weights: bool = True, return_contacts: bool = False):
        if return_contacts:
        ...
        # if 'token_dropout' not in self.args:
        ...
        # repr_layers:Set = set(repr_layers)
        hidden_representations: List[Tensor] = [x]
        # if 0 in repr_layers:
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            # else:
            #     hidden_representations.append(torch.zeros_like(x,device=x.device))
            if need_head_weights:
                # (H, B, T, T) => (B, H, T, T)
                attn_weights.append(torch.transpose(attn, 1, 0))
                # # (H, B, T, T) => (B, T, T)
                # attn_weights.append(torch.transpose(attn, 1, 0))
        ...
        # result = {"logits": x, "representations": hidden_representation}
        # if need_head_weights:
        # attentions: B x L x H x T x T
        attentions = torch.stack(attn_weights, 1)
        # attentions = torch.concat(attn_weights, -1)
        ...
        # result_tuple = namedtuple('result_tuple', ['attentions', 'logits', 'representations'])
        return attentions, hidden_representation, x
        # return result_tuple(attentions=attentions, logits=x, representations=hidden_representation)
