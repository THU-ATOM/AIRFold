...
class AxialTransformerLayer(nn.Module):
    ...
    def forward(
        ...
        """
        row_attn = partial(self.row_self_attention,
                           self_attn_mask=self_attn_mask,
                           self_attn_padding_mask=self_attn_padding_mask,
                           )
        col_attn = partial(self.column_self_attention,
                           self_attn_mask=self_attn_mask,
                           self_attn_padding_mask=self_attn_padding_mask,
                           )
        if x.size(0) > 1:
            x, row_attn = checkpoint(row_attn, x)
            x, _ = checkpoint(col_attn, x)
        else:
            x, row_attn = row_attn(x)
            x, _ = col_attn(x)
        ...
        #     self_attn_padding_mask=self_attn_padding_mask,
        # )
        # x, _ = self.column_self_attention(
        #     x,
        #     self_attn_mask=self_attn_mask,
        #     self_attn_padding_mask=self_attn_padding_mask,
        # )
