...
class MSATransformer(nn.Module):
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            #     if layer_idx == 0:
            #         x = x.cuda(1)
            #         padding_mask = None if padding_mask is None else padding_mask.cuda(1)
            # elif layer_idx < 6:
            #     layer = layer.cuda(2)
            #     if layer_idx == 3:
            #         x = x.cuda(2)
            #         padding_mask = None if padding_mask is None else padding_mask.cuda(2)
            # elif layer_idx < 9:
            #     layer = layer.cuda(3)
            #     if layer_idx == 6:
            #         x = x.cuda(3)
            #         padding_mask = None if padding_mask is None else padding_mask.cuda(3)
            # else:
            #     # layer = layer.cuda(0)
            #     if layer_idx == 9:
            #         x = x.cuda(0)
            #         padding_mask = None if padding_mask is None else padding_mask.cuda(0)
            x = layer(
                x,
            ...
            if need_head_weights:
                ...
                row_attn_weights.append(row_attn.permute(1, 0, 2, 3).cuda(0))
            if (layer_idx + 1) in repr_layers:
