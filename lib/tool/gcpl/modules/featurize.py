...
from lib.tool.gcpl.modules.Tri_D import tri_location_D
...
def DC(cb_d):
    ...
    contact_mask = cb_d < 15
    L = len(cb_d)
    dist_order = []
    contact_order = []

    for i in range(len(cb_d)):
        N = sum(list(contact_mask[i, :])) - 1  # exclude oneself
        deltaS = 0
        dist_row = []

        for j, va in enumerate(contact_mask):
        ...
        contact_order.append(con)
        dist_order.append(dist_row)

    contact_order = np.expand_dims(np.array(contact_order), axis=1)
    dist_order = np.expand_dims(np.array(dist_order), axis=-1)
    norm_co = contact_order / np.linalg.norm(contact_order)
...
def process(filename, outfile, verbose=True):
    ...
    # prop = np.vstack((prop,usr))[:52,]

    cb_d = maps[:, :, 0]
    [norm_co,scale_do]=DC(cb_d)
    L = cb_d.shape[0]
    model_coords, _ = process_model(filename)
    cb_coords = rearrange(
        model_coords,
        "b (l a) d -> b l a d",
        l=L,
    ...
    np.savez_compressed(outfile,
        ...
        norm_co = norm_co.astype(np.float16),
